...
def recommend_top_k(model, data, k):
    ...
    top_k_items = (
        df_pred_score.groupby(DEFAULT_USER_COL, as_index=False)
        .apply(lambda x: x.nlargest(k, DEFAULT_PREDICTION_COL))
        .reset_index(drop=True)
    )
    top_k_items["rank"] = (
        top_k_items.groupby(DEFAULT_USER_COL, sort=False).cumcount() + 1
    )
    top_k_recommend = top_k_items.pivot_table(
        values=DEFAULT_ITEM_COL, index=DEFAULT_USER_COL, columns="rank"
    )

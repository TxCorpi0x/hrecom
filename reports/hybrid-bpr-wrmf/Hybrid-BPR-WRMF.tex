...
\title{Hybrid Improvement on Bayesian Rank Prediction and WR-MF}
%
%
...
% The paper headers
\markboth{Hybrid Improvement on Bayesian Rank Prediction and WR-MF, VOL.~01, NO.~01, JANUARY~2024}%
...
through enhancements utilizing Cosine and Jaccard Similarity.
...
Given \(Sr(u_i)\) as the set of similar items for a particular user \(u_i\) with ranking higher than \(min_r\), 
...
higher than \(min_r\)).
\[
  Sr(u_i) = \left\{ \begin{array}{ll}
    Sims(m_i) & \mbox{if $r_i \geq min_r$};\\
      & \mbox{if $r_i < 3.0$}.\end{array} \right.
\]

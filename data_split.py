...
def leave_one_last(data, validation=False):
    ...
    if validation:
        df_split.loc[
            df_split.groupby([DEFAULT_USER_COL]).head(2).index, DEFAULT_SPLIT_FLAG
        ] = "val"

    df_split.loc[
        df_split.groupby([DEFAULT_USER_COL]).head(1).index, DEFAULT_SPLIT_FLAG
    ] = "test"
    df_split = df_split.sort_values([DEFAULT_USER_COL, DEFAULT_ITEM_COL]).reset_index(
        drop=True
    )
...
def temporal_user_split(
    data, test_size=DEFAULT_TEST_SIZE, val_size=DEFAULT_VAL_SIZE, validation=False
):
    """Temporal user split
    ...
    for u in tqdm(users):
        ...
        if validation:
            ...
            df_split.loc[
                user_index[(train_size - val_split_size) : train_size],
                DEFAULT_SPLIT_FLAG,
            ] = "val"
...
def split_data(
    ...
    if split_strategy == "leave_one_last":
        ...
        df_split = random_split_by_user(
            data, test_size, val_size, validation, random_state
        )
    else:
